	...
	public function __construct() {
		parent::__construct(
	 		'Bannerlink_Widget', // Base ID
                        __( 'Banner/Logo Link', 'tf2013' ),
			array( 'description' => __( 'Schalten von Link mit einem Logo oder Banner', 'tf2013' ), ) // Args
		);
	}
	public function form($instance) {
	    $defaults = array(
	    ...
	    ?>
		    <p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Titel:','tf2013'); ?></label>
		    ...
		    <p>
			<label for="<?php echo $this->get_field_id('url'); ?>"><?php _e('Ziel-URL:','tf2013'); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id('url'); ?>"
		     ...
		     <p>
			<label for="<?php echo $this->get_field_id('image_url'); ?>"><?php _e('Bild-URL:','tf2013'); ?>
			       ...
			       type="text" value="<?php echo esc_attr($image_url); ?>" />
                        	...
                        	<input type="hidden" id="<?php echo $this->get_field_id('image_id'); ?>"
                                ...
                                <input
                                    ...
                                    class="upload_image_button" value="<?php _e('Hochladen / Ausw&auml;hlen', 'tf2013'); ?>" type="button" />

			    <br /><?php _e('Gebe eine URL zu einem Bild ein oder verwende die Mediathek um es hochzuladen oder um ein vorhandenes Bild auszuw&auml;hlen.', 'tf2013'); ?>
                        </label>
		    </p>
		    <?php
	}
	...
	public function widget($args, $instance) {
	    ...
	    echo $before_widget;
            if ((isset($url)) && (strlen($url)>0))
                echo '<a href="'.$url.'" class="bannerlink">';
	    if ($image_url) {
		if ($image_height > 0) {
